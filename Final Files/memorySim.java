...
public class memorySim {
	...
	private static void allocateMemory(long time) {
	    ...
	    for (int i = 0; i < limit; i++) {
	        ...
	        if (framelist.canPlaceInMemory(p, policy)) {	//Different for PAG
	            System.out.printf("%sMM moves Process %d to memory\n",
	            		getAnnouncedTime(time),
	            		p.getProcessId());

	            framelist.fitProcessinMemory(p, policy, fitAlgorithm);  //This is different for all policies and types

	            p.setIsActive(true);
	            p.setArrivalInMemory(time);
	            p_queue.dequeuProcessAtIndex(i);
	            p_queue.printQueue();
	            framelist.printFrameList(policy, fitAlgorithm);	//This is different for all policies and types
	        }
	    }		
	}
	...
	private static void terminateProcess(long time) {
	    ...
	    for (int i = 0; i < numOfProcesses; i++) {
	        ...
	        if (p.getIsActive() && (timeInMemory >= p.getArrivalInMemory())){
	            ...
	            framelist.printFrameList(policy, fitAlgorithm); //This is different for all policies
	        }
	    }
	}
